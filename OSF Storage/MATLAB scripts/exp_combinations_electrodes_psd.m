...
% Define the number of electrodes and frequency bins. 
numElectrodes = 19;
numSubjects = 25;
numBins = 21;

% Initialize a cell array to store the results for all combinations
avg_psd_combinations_bf = cell(2^numElectrodes, 1);
avg_psd_combinations_mw = cell(2^numElectrodes, 1);

% Iterate through all 2^n combinations of electrodes
for combination = 1:(2^numElectrodes)
    % Convert the combination number to binary and use it as a mask
    electrodeMask = dec2bin(combination - 1, numElectrodes) - '0';
    
    % Find the indices of the selected electrodes
    selectedElectrodes = find(electrodeMask);
    
    disp(selectedElectrodes);

    % Reshape and calculate the average PSD for the selected electrodes
    reshaped_psd_value_bf = reshape(cell2mat(psd_value_bf(:)'), [numSubjects, numElectrodes, numBins]);
    reshaped_psd_value_mw = reshape(cell2mat(psd_value_mw(:)'), [numSubjects, numElectrodes, numBins]);
    
    % Use only the selected electrodes for the average calculation
    avg_psd_combinations_bf{combination} = squeeze(mean(reshaped_psd_value_bf(:, selectedElectrodes, :), 2));
    avg_psd_combinations_mw{combination} = squeeze(mean(reshaped_psd_value_mw(:, selectedElectrodes, :), 2));
    
    % Store the combination details, if needed
    combinationDetails{combination} = selectedElectrodes;
end

%after running this file,I saved it as
%exp_after_combinations_electrodes_psd.mat
